      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '20px 16px 100px 16px' }}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          <input
            ...
            placeholder="Add an item..." value={newItem}
            onChange={e => setNewItem(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && newItem.trim()) { saveItems([...groceryItems, { id: Date.now(), text: newItem.trim(), checked: false, category: 'Other' }]); setNewItem(''); }}}
          />
          <button onClick={() => { if (newItem.trim()) { saveItems([...groceryItems, { id: Date.now(), text: newItem.trim(), checked: false, category: 'Other' }]); setNewItem(''); }}}
        ...
        {groceryItems.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
            <button onClick={() => saveItems([])} style={{ background: 'none', border: 'none', fontSize: '12px', color: '#BDC2B4', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', fontWeight: '400' }}>Clear all</button>
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {items.map(item => (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <span style={{ flex: 1, fontSize: '14px', fontWeight: '300', color: '#404F43' }}>{item.text}</span>
                    <button onClick={() => saveItems(groceryItems.filter(i => i.id !== item.id))}
                      style={{ background: 'none', border: 'none', color: '#BDC2B4', fontSize: '18px', cursor: 'pointer', padding: '0 4px', lineHeight: 1 }}>×</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          {checked.length > 0 && (
            <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <p style={{ fontSize: '11px', fontWeight: '600', color: '#BDC2B4', letterSpacing: '0.5px', margin: '0 0 12px 0' }}>COMPLETED</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {checked.map(item => (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <input type="checkbox" checked={true}
                      ...
                      style={{ accentColor: '#5AA0B4', width: '16px', height: '16px' }} />
                    <span style={{ flex: 1, fontSize: '14px', fontWeight: '300', color: '#BDC2B4', textDecoration: 'line-through' }}>{item.text}</span>
                    <button onClick={() => saveItems(groceryItems.filter(i => i.id !== item.id))}
                      style={{ background: 'none', border: 'none', color: '#BDC2B4', fontSize: '18px', cursor: 'pointer', padding: '0 4px', lineHeight: 1 }}>×</button>
                  </div>
                ))}
